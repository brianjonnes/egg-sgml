...
class tgc_module_test__a {
	...
	public $testpath;
	public $modules;
	function __construct($path,$env,$testpath,$modules) {
		$this->path = $path;
		$this->env = $env;
...
class mtgc_module_test {
	...
	function initialize($path,$env) {
		...
		$this->c = new tgc_module_test__a($path,$env,$_SERVER['DOCUMENT_ROOT'],$this->modules);
	}
	function start( $q ) {
		return 0; }
	function repeat( $q ) {
	...
	function consume( $q, $end, $w ) {
		...
		if( $w->nodeName == 'module_list' ) {
			...
			while( ( $a = readdir($n) ) !== false ) {
				if( strtoupper( substr($a,strlen($a)-4) ) == strtoupper( $this->env->file_ext ) ) {
